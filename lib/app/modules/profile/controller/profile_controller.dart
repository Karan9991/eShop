...
class ProfileController extends GetxController {
  ...
  updateUserProfile(context) async {
    ...
    Map<String, String>? body = {
      ...
      'country_code': countryCodeController.text.toString() == '' ? authController.countryCode.toString() : countryCodeController.text.toString(),
    };
      ...
      if (response != null && response.statusCode == 200) {
        ...
        if (message['errors']['new_password'] != null) {
          customSnackbar('CHANGE_PASSWORD'.tr,
              message['errors']['new_password'][0].toString().tr, AppColor.error);
        }
        if (message['errors']['old_password'] != null) {
          customSnackbar('CHANGE_PASSWORD'.tr,
              message['errors']['old_password'][0].toString().tr, AppColor.error);
        }
        isLoading(false);
